      ...
      <CategoryMenuWrapper id="mobileMenu">
        <MainMenuNav>
          <CategoryMenuItem key={'account icon'}>
            <CategoryMenuLink onClick={toggleAccountMenu}>
              <Icon
                onClick={toggleAccountMenu}
                src="/GYB-account.svg"
                alt="account icon"
              />{' '}
              Account{tabs.length ? <Down onClick={openSubNav}></Down> : ''}
            </CategoryMenuLink>
            {tabs.length ? (
              <SubMenuNav>
                {tabs.map((d) => {
          ...
          <CategoryMenuItem key={'basket icon'}>
            <CategoryMenuLink onClick={basketCheck}>
            ...
            return (
              <CategoryMenuItem key={c.id}>
                <CategoryMenuLink
                  key={'link-' + c.id}
                  onClick={() => {
                    history.push('/products/' + c.id);
                    hideMobileMenu();
                  }}
                >
                  {c.name} <Down onClick={openSubNav}></Down>
                </CategoryMenuLink>
                <SubMenuNav>
                  {subCategories.map((d) => {
                    d.id = d.id.replace('/', '-');
