...
import { ListWrapper, HiddenStyledTab, StyledTabPanel, StyledTabs, StyledTabListHidden, ShopperWrapper, ShopperName, ShopperWrapperSmall } from './DonorOrdersList.styles';
import { getDonorItems, updateItem } from '../../../services/items';
import { H2, Button } from '../../atoms';
import { getUser } from '../../../services/user';
import { getLocation } from '../../../services/locations';
import { sendAutoEmail, getDate, tabList, donorItemsOrdering } from "../../../utils/helpers";
    ...
    const markAsSent = (itemsList) => {
        confirm({
            ...
            onOk() {
                ...
                itemsList.forEach((l) => {
                    setItemsToSend(itemsToSend.filter(j => {
                    ...
                    if (typeof l.sendVia === 'string') {
                        ...
                        return updateItem(l._id, { 'status': 'shipped-to-gyb', 'statusUpdateDates.gybShippedDate': getDate(), packageId: uuid }, token)
                    } else {
                        //send via does not exist (location not assigned or shopper has shared address
                        sendAutoEmail('item_on_the_way', l.shopperId, [l]);
                        return updateItem(l._id, { 'status': 'shipped-to-shopper', 'statusUpdateDates.shopperShippedDate': getDate(), packageId: uuid }, token)
                    }
                })
            }
          });
                        ...
                        {itemsToSend.map((item) => {
                            ...
                            return (
                                <ShopperWrapper>
                                    ...
                                    {item.items && item.items.length && <Button primary right small onClick={() => markAsSent(item.items)}>Mark as Sent</Button>}
                                </ShopperWrapper>
                                )
                        }
                        )
