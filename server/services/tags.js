...
const updateItemWithTags = async (tag, itemId) => {
  ...
  try {
      await Item.findOneAndUpdate({ _id: itemId, "tags": { "$ne": tag } }, { $push : { tags: tag }})
...
const updateUserWithTags = async (tag, userId) => {
  ...
  try {
      await User_.User.findOneAndUpdate({ _id: userId, "tags": { "$ne": tag } }, { $push : { tags: tag }})
...
const deleteTagFromUser = async (tag, userId) => {
  ...
  try {
      await User_.User.findOneAndUpdate({ _id: userId }, { $pull : { tags: tag }})
